...
class ModbusBleBridge : public Component, public ble_client::BLEClientNode {
 ...
 protected:
  ...
  uint32_t ble_response_timeout_ms_ = 2000;  // Timeout waiting for BLE notify
  ...
  unsigned long waiting_since_ = 0;     // When we started waiting for BLE response
  unsigned long last_wait_log_ = 0;     // For throttling "waiting" logs
